				...
				//System.out.println(path);
					...
					try {	
						...
						if(experimentsType.equals("UCT_TESTING"))
						{
							...
							UCTOutputFile=new PrintStream(new File(INPUT_FOLDER_PATH+"Bandit/UCT/N"+dataCategory+dataType[y]+"/t"+timeslot+"/"+distance[m]+"m"+"/C"+ucb_ScalerMatrix+"/cost"+costPerMetre+"/"+Centralized.testDataType+"/"+Centralized.noOfTimesRecommendationSegment+"/TestC"+testingUcb_scaler+"/TestCost"+testingCost+"/FOutput"+"-"+trajectories+".csv")); 
							uct.DemoUCT1.main(null,phase,TRAIN_INPUT_SEGMENT_FILE,TEST_INPUT_SEGMENT_FILE,UCTOutputFile,predictionSegments,pathOfLearntParameters,distance[m],noOfTimesRecommendationSegment);
							UCTOutputFile.close();
						...
						if(experimentsType.equals("BASELINE"))
						{
							...
							PrintStream BaseOutputFile=new PrintStream(new File(INPUT_FOLDER_PATH+"Bandit/Baseline/N"+dataCategory+dataType[y]+"/t"+timeslot+"/"+distance[m]+"m/"+testDataType+"/"+noOfTimesRecommendationSegment+"/FOutput.csv"));
